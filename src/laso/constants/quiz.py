"""Quiz domain constants — eligibility thresholds, field keys, step config."""

# ── Eligibility ──────────────────────────────────────────────────────────────
BMI_THRESHOLD_STANDARD    = 30.0   # eligible on BMI alone
BMI_THRESHOLD_COMORBIDITY = 27.0   # eligible if BMI ≥ this AND has comorbidity
COMORBIDITIES = frozenset({"t2dm", "prediabetes", "hypertension", "dyslipidemia",
                            "nafld", "pcos", "hypothyroid", "sleep_apnea"})

# ── Quiz steps (inlined — avoids any file-system lookup inside Lambda) ────────
QUIZ_STEPS: list[dict] = [
  {
    "id": "personal",
    "stepNumber": 1,
